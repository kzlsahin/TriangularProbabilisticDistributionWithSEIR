...
def runByCountryName(country_name, prefix = "", suffix=""):
    ...
    DailyRecovered = pd.DataFrame(DailyRecovered).rolling(7).sum()/7
    DailyRecovered = np.nan_to_num(DailyRecovered .to_numpy().flatten())
    
    DailyCases = pd.DataFrame(DailyCases).rolling(7).sum()/7
...
#result = runByCountryName("Brazil")
#runByCountryName("SKorea", prefix="", suffix = "shortOptimized")

for name in country_names:
    runByCountryName(name, prefix="27012023", suffix = "longOptimized")
